...
model=changeRxnBounds(model,'EX_4hpro_DC(e)',1,'l');


%Vitamins
model=changeRxnBounds(model,'EX_btn(e)',-0.1,'l');
model=changeRxnBounds(model, 'EX_chol(e)',-0.1,'l');
model=changeRxnBounds(model,'EX_pnto_R(e)',-0.01,'l');
model=changeRxnBounds(model,'EX_dhf(e)',-0.1,'l');
model=changeRxnBounds(model,'EX_4abz(e)',-0.1,'l');
model=changeRxnBounds(model,'EX_pydxn(e)',-0.1,'l');  
model=changeRxnBounds(model,'EX_rbf(e)',-0.1,'l'); 
model=changeRxnBounds(model,'EX_thm(e)',-0.01,'l'); 
model=changeRxnBounds(model,'EX_nmn(e)',-0.1,'l');
model=changeRxnBounds(model, 'EX_inost(e)',-0.1,'l');  
